...
def register(): 
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)  
        db.session.commit()
        flash('Account for  {}!  created '.format(form.email.data), "success" )
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            flash('Login Unsccessful. Register first, or please check email and password', 'danger')    
...
def reset_token(token):
    ...
    if user is None:
        flash('token is invalide or has expire', 'warning')
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Your password have been updated, you can now login', "success" )
        return redirect(url_for('users.login'))
